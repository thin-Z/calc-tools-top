// api/test/likes.test.js
// 点赞 API 单元测试（零依赖：node:test + node:assert + 本地 KV 模拟服务器）
// 运行：node --test api/test/likes.test.js  或  node --test api/test/likes.test.js api/test/clicks.test.js
//
...
test('GET 缺少 toolId → 403', async () => {
  const r = await httpCall(handlerPort, 'GET', '/api/likes');
  assert.strictEqual(r.status, 403);
});

test('GET 非法 toolId（纯特殊字符）→ 400', async () => {
  const r = await httpCall(handlerPort, 'GET', '/api/likes?toolId=%21%21%21');
  assert.strictEqual(r.status, 400);
});

test('GET 返回当前计数', async () => {
  await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, { toolId: 'get-tool', action: 'like' });
  const r = await httpCall(handlerPort, 'GET', '/api/likes?toolId=get-tool');
...
test('mock 拒绝旧格式 /incr/{key}/{delta}（防格式回归）', async () => {
  // 2026-08-17 教训：旧格式被 Upstash 解析为 INCR key delta 报参数错误，
  // 计数静默丢失。mock 必须同样拒绝，否则测试无法拦住格式回归。
  const r = await httpCall(mock.port, 'GET', '/incr/like:tool:old-format/1', {});
  assert.strictEqual(r.status, 400);
});
...
test('POST 取消点赞 → 计数 -1 且不低于 0', async () => {
  await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, { toolId: 'unlike-tool', action: 'like' });
  const r = await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, { toolId: 'unlike-tool', action: 'unlike' });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.body.count, 0);
  assert.strictEqual(r.body.liked, false);
  // 再取消一次，计数保持 0
  const r2 = await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, { toolId: 'unlike-tool', action: 'unlike' });
  assert.strictEqual(r2.body.count, 0);
});

test('POST 缺少 toolId → 400', async () => {
  const r = await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, {});
  assert.strictEqual(r.status, 400);
});

test('POST 超过写限速（20/min）→ 429', async () => {
  // 使用独立 IP，避免影响其它用例；每个 toolId 唯一，绕过每日点赞防刷
  const rlHeaders = { 'content-type': 'application/json', 'x-forwarded-for': '203.0.113.50' };
  let last;
  for (let i = 0; i < 21; i++) {
    last = await httpCall(handlerPort, 'POST', '/api/likes', rlHeaders, { toolId: 'ratelimit-' + i, action: 'like' });
  }
  assert.strictEqual(last.status, 429);
});

test('OPTIONS → 200', async () => {
  const r = await httpCall(handlerPort, 'OPTIONS', '/api/likes');
  assert.strictEqual(r.status, 200);
});

test('不支持的 method → 405', async () => {
  const r = await httpCall(handlerPort, 'DELETE', '/api/likes');
  assert.strictEqual(r.status, 405);
});

test('点赞计数 key 写入后刷新 365 天 TTL', async () => {
  const toolId = 'ttl-tool';
  await httpCall(handlerPort, 'POST', '/api/likes', JSON_HEADERS, { toolId, action: 'like' });
  const found = mock.expires.find((e) => e.key === 'like:tool:' + toolId && e.seconds === 31536000);
  assert.ok(found, '应记录 expire like:tool:' + toolId + ' 31536000，实际记录: ' + JSON.stringify(mock.expires));
});
