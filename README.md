...
| 安全 | **CSP 全站硬化**：script-src / style-src 无 `unsafe-inline`（`js/csp-events.js` 委托层 + `js/inline/*.js` 外链化），img-src 白名单化；`verify-site.mjs` 29 项断言守护 |
...
# 2) 集成校验 29 项断言：header/footer 字节一致 + JSON-LD + AdSense 唯一性
