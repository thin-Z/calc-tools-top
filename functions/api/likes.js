...
export async function onRequest(context) {
  ...
  // KV binding 未配置时降级返回 0
  if (!env || !env.LIKES) {
    return new Response(JSON.stringify({ error: 'KV binding not configured', count: 0 }), {
      status: 200,
      headers: {
  ...
  if (method === 'POST') {
    ...
    let rateCount;
    try {
      rateCount = await env.LIKES.get(rateKey);
    } catch (e) {
      rateCount = null;
    }
    ...
    try {
      ...
      // rate limit tracking failed, continue
    }
